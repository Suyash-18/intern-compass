      ...
      <div className="space-y-8 animate-fade-in">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <Card className="border-l-4 border-l-status-approved">
            ...
            <CardContent><div className="text-3xl font-bold text-status-approved">{stats.completed}</div></CardContent>
          </Card>
          <Card className="border-l-4 border-l-status-pending">
            ...
            <CardContent><div className="text-3xl font-bold text-status-pending">{stats.pending}</div></CardContent>
          </Card>
          <Card className="border-l-4 border-l-status-locked">
            ...
            <CardContent><div className="text-3xl font-bold text-status-locked">{stats.locked}</div></CardContent>
          </Card>
        </div>

        <div>
          <h2 className="text-xl font-bold mb-4">Your Tasks</h2>
          {currentInternTasks.length === 0 ? (
            <Card>
            ...
            <div className="space-y-4">
              {currentInternTasks.map((task, index) => (
                <TaskCard key={task.id} task={task} index={index} onSubmit={handleSubmit} />
              ))}
            </div>
          )}
        </div>
