...
import { ClipboardList, Check, X, MessageSquare, User, FileText, Image, Archive, Download, Eye, Paperclip, Plus } from 'lucide-react';
...
export default function AdminTasks() {
  ...
  const { interns, reviewTask, isLoading } = useInterns();
  ...
  const reviewedTasks: PendingTask[] = interns.flatMap((intern) =>
    intern.tasks.map((task, index) => ({ intern, task, taskIndex: index }))
      .filter((item) => (item.task.status === 'approved' || item.task.status === 'rejected') && item.task.reviewedAt)
  ).sort((a, b) => new Date(b.task.reviewedAt || '').getTime() - new Date(a.task.reviewedAt || '').getTime()).slice(0, 10);

  const handleReview = (type: 'approve' | 'reject') => setReviewType(type);

  const submitReview = async () => {
    if (!selectedTask || !reviewType) return;
    await reviewTask(selectedTask.intern.id, selectedTask.task.id, reviewType === 'approve' ? 'approved' : 'rejected', feedback);
    toast({
  ...
  const handlePreview = (attachment: TaskAttachment) => {
    if (attachment.type === 'image' || attachment.type === 'pdf') {
      ...
      toast({ title: 'Preview not available', description: 'Please download the file instead.' });
    }
  };
      ...
      <div className="space-y-6 animate-fade-in">
        <div className="bg-foreground text-background rounded-2xl p-6 md:p-8">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div>
              <h1 className="text-2xl md:text-3xl font-bold flex items-center gap-3"><ClipboardList className="h-8 w-8" />Task Review</h1>
              <p className="text-background/70 mt-2">Review and approve intern task submissions</p>
            </div>
            <Button onClick={() => navigate('/admin/tasks/new')} className="bg-primary hover:bg-primary/90"><Plus className="mr-2 h-4 w-4" />Add New Task</Button>
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
          <Card className="border-l-4 border-l-status-pending">
            ...
            <CardContent><div className="text-3xl font-bold text-status-pending">{pendingTasks.length}</div></CardContent>
          </Card>
          <Card className="border-l-4 border-l-status-approved">
            ...
            <CardContent><div className="text-3xl font-bold text-status-approved">{reviewedTasks.filter((t) => t.task.status === 'approved').length}</div></CardContent>
          </Card>
          <Card className="col-span-2 md:col-span-1 border-l-4 border-l-primary">
            ...
            <CardContent><div className="text-3xl font-bold">{interns.reduce((acc, i) => acc + i.tasks.length, 0)}</div></CardContent>
          </Card>
        </div>
        ...
        <Tabs defaultValue="pending" className="space-y-4">
          <TabsList className="bg-muted p-1">
            <TabsTrigger value="pending" className="gap-2 data-[state=active]:bg-primary data-[state=active]:text-primary-foreground">
              Pending
              {pendingTasks.length > 0 && <span className="bg-status-pending text-status-pending-foreground text-xs px-2 py-0.5 rounded-full">{pendingTasks.length}</span>}
            </TabsTrigger>
            <TabsTrigger value="reviewed" className="data-[state=active]:bg-primary data-[state=active]:text-primary-foreground">Recently Reviewed</TabsTrigger>
          </TabsList>

          <TabsContent value="pending" className="space-y-4">
            {pendingTasks.length === 0 ? (
              <Card><CardContent className="py-12 text-center"><ClipboardList className="h-12 w-12 mx-auto text-muted-foreground mb-4" /><h3 className="text-lg font-medium">No pending tasks</h3><p className="text-muted-foreground mt-1">All submitted tasks have been reviewed.</p></CardContent></Card>
            ) : (
                ...
                <Card key={`${item.intern.id}-${item.task.id}`} className="hover:shadow-lg transition-shadow">
                  ...
                  <CardContent>
                    ...
                    <div className="flex items-center justify-between">
                      <p className="text-xs text-muted-foreground">Submitted: {item.task.submittedAt ? new Date(item.task.submittedAt).toLocaleDateString() : 'N/A'}</p>
                      <Button onClick={() => setSelectedTask(item)}><MessageSquare className="mr-2 h-4 w-4" />Review Task</Button>
                    </div>
                  </CardContent>
                </Card>
              ))
          ...
          <TabsContent value="reviewed" className="space-y-4">
            {reviewedTasks.length === 0 ? (
              <Card><CardContent className="py-12 text-center"><ClipboardList className="h-12 w-12 mx-auto text-muted-foreground mb-4" /><h3 className="text-lg font-medium">No reviewed tasks yet</h3></CardContent></Card>
            ) : (
                  ...
                  <CardHeader>
                    <div className="flex items-start justify-between gap-4">
                      <div>
                        ...
                        <div className="flex items-center gap-2 mt-2"><User className="h-4 w-4 text-muted-foreground" /><span className="text-sm text-muted-foreground">{item.intern.profile.name}</span></div>
                      </div>
                      <StatusBadge status={item.task.status} />
                    </div>
                  </CardHeader>
        ...
        <Dialog open={!!selectedTask} onOpenChange={() => { setSelectedTask(null); setFeedback(''); setReviewType(null); }}>
            ...
            <div className="space-y-4">
              ...
              {!reviewType ? (
                <div className="flex gap-3">
                  <Button variant="outline" className="flex-1 h-12 border-2 border-destructive text-destructive hover:bg-destructive hover:text-destructive-foreground" onClick={() => handleReview('reject')}>
                    <X className="mr-2 h-5 w-5" />Reject
                  </Button>
                  <Button className="flex-1 h-12 bg-status-approved hover:bg-status-approved/90" onClick={() => handleReview('approve')}>
                    <Check className="mr-2 h-5 w-5" />Approve
                  </Button>
                </div>
              ) : (
                <DialogFooter>
                  <Button variant="outline" onClick={() => setReviewType(null)}>Back</Button>
                  <Button onClick={submitReview} className={reviewType === 'approve' ? 'bg-status-approved hover:bg-status-approved/90' : 'bg-destructive hover:bg-destructive/90'}>
        ...
        <Dialog open={!!previewAttachment} onOpenChange={() => setPreviewAttachment(null)}>
          <DialogContent className="max-w-4xl max-h-[90vh]">
            <DialogHeader><DialogTitle>{previewAttachment?.name}</DialogTitle></DialogHeader>
            <div className="flex items-center justify-center min-h-[400px]">
              {previewAttachment?.type === 'image' ? (
                <img src={previewAttachment.url} alt={previewAttachment.name} className="max-w-full max-h-[70vh] object-contain rounded-lg" />
